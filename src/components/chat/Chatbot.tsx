...
export default function Chatbot({ initialMessage, ctx }: ChatbotProps) {
  ...
  const handleSendMessage = async () => {
    ...
    if (!sessionId || currentQuestionId === null) {
      setLastError('세션 또는 질문 ID가 준비되지 않아 평가를 생략했습니다.');
  ...
  const onEndInterview = async () => {
    try {
      ...
      const data: FinevalResponse = await InterviewAPI.fineval({
        session_id: Number(sessionId),
      });

      setLastResponse({ type: 'fineval', result: data });
      ...
      setLastError('fineval 실패: ' + (e?.message || e));
      navigate('/leaderboard', {
        state: {
          interview_log: [],
